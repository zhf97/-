...
Page({
  ...
  submit: function (e) {
    var barrelid= wx.getStorageSync('id');
    console.log('barrelid',barrelid);
    if(barrelid==1)
    wx.setStorageSync('barrel1',this.data.cart.list);
    if (barrelid == 2)
      wx.setStorageSync('barrel2', this.data.cart.list);
    if (barrelid == 3)
      wx.setStorageSync('barrel3', this.data.cart.list);
    if (barrelid == 4)
      wx.setStorageSync('barrel4', this.data.cart.list);
    if (barrelid == 5)
      wx.setStorageSync('barrel5', this.data.cart.list);
    if (barrelid == 6)
      wx.setStorageSync('barrel6', this.data.cart.list);
    if (barrelid == 7)
      wx.setStorageSync('barrel7', this.data.cart.list);
    if (barrelid == 8)
      wx.setStorageSync('barrel8', this.data.cart.list);

   /* wx.request
      ({
        url: 'https://www.jl-lagrange.com.cn/cxsy/order.php', //接口地址
        data: { list: JSON.stringify(this.data.cart.barrel) },
        header: {
